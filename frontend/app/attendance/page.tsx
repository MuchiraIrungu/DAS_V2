...
import { 
    Bell, CalendarCheck, History, Printer, SendHorizonal, 
    SquareArrowDownLeft, CheckCircle, XCircle, Loader2, LogOut 
} from "lucide-react";
...
export default function Attendance() {
    ...
    // Fetch teacher info + their classes only
    useEffect(() => {
        const fetchTeacherData = async () => {
            try {
                const meRes = await fetch(`${API_PATH}/api/auth/me/`, { credentials: "include" });
                const meData = await meRes.json();

                if (meData.success && meData.user) {
                    const user = meData.user;
                    setTeacherName(user.full_name || user.username || "Teacher");
                    if (user.school) {
                        setTeacherSchool(user.school.name || user.school);
                    }
                }

                // Fetch only classes assigned to this teacher
                const classesRes = await fetch(`${API_PATH}/api/classes/?teacher_only=true`, {
                    credentials: "include"
                });
                const classesData = await classesRes.json();

                if (classesData.success) {
                    const list: ClassOption[] = (classesData.data.results ?? classesData.data).map(
                ...
                console.error("Failed to fetch teacher data/classes", err);
            }
        };
    ...
    // Fetch students for the selected class (backend already restricts to teacher's classes)
    const fetchStudents = useCallback(async () => {
        ...
        try {
            ...
            const mapped: AttendanceStudent[] = rawList.map((s:any) => ({
                id: s.id,
    ...
    // Logout function (same logic as Sidebar)
    ...
    const handleSubmit = async () => {
        ...
        const submitData = {
            ...
            date: selectedDate,
            marked_by: 1, // TODO: Replace with real teacher ID from /api/auth/me/
            ...
            <section className="nav min-h-[10vh] bg-gray-50 shadow-xl justify-between items-center flex flex-row p-3 px-4 sm:px-6 lg:px-10 sticky top-0 z-50">
                ...
                <div className="flex flex-row gap-3 lg:gap-6 justify-center items-center">
                    <div className="flex flex-col justify-center items-end sm:flex">
                        ...
                        <span className="text-gray-500 text-xs lg:text-sm">Teacher</span>
                    </div>
                    <Image
                    ...
                    <button
                        ...
                        disabled={loggingOut}
                        className="flex flex-row items-center gap-2 text-gray-900 hover:text-red-600 transition-colors disabled:opacity-60"
                    >
