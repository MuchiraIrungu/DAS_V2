...
// ─── Logout overlay (mirrored from Sidebar) ───────────────────────────────────

function LogoutOverlay() {
    return (
        <div
            className="fixed inset-0 z-[9999] flex flex-col items-center justify-center"
            style={{ backgroundColor: "#d4e3f3" }}
        >
            <div className="flex flex-col items-center gap-6">
                <div className="relative">
                    ...
                    <span
                        ...
                        style={{ backgroundColor: "rgba(30, 136, 229, 0.18)", animationDuration: "1.2s" }}
                    />
                </div>
                <div className="relative w-12 h-12">
                    <svg className="animate-spin w-12 h-12" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 48 48">
                        <circle cx="24" cy="24" r="20" stroke="#b7d9f5" strokeWidth="4" fill="none" />
                        <path d="M44 24a20 20 0 00-20-20" stroke="#1e88e5" strokeWidth="4" strokeLinecap="round" fill="none" />
                    </svg>
                </div>
                <div className="text-center">
                    <p className="text-[#1e3a5f] font-semibold text-base tracking-wide">Signing you out...</p>
                    <p className="text-[#5a7fa8] text-sm mt-1">See you next time</p>
                </div>
                <div className="flex gap-2 mt-1">
                    {[0, 1, 2].map((i) => (
                        <span key={i} className="w-2 h-2 rounded-full bg-[#1e88e5] animate-bounce" style={{ animationDelay: `${i * 0.18}s` }} />
                    ))}
                </div>
            </div>
        </div>
...
export default function Attendance() {
    ...
    // ── Step 1: Fetch current user from /me ──────────────────────────────────
    useEffect(() => {
        const fetchMe = async () => {
            try {
                const res = await fetch(`${API_PATH}/api/auth/me/`, { credentials: "include" });
    ...
    // ── Step 2: Populate classes based on role ───────────────────────────────
    useEffect(() => {
        if (!currentUser) return;

        if (isTeacher && currentUser.teacher_profile_data) {
            // Teacher: only their assigned classes, locked
            const teacherClasses = currentUser.teacher_profile_data.classes.map((c) => ({
                id: c.id,
                name: c.name,
            }));
            setClasses(teacherClasses);
            if (teacherClasses.length > 0) setSelectedClasses(teacherClasses[0]);
        } else if (isAdmin) {
    ...
    // ── Step 3: Fetch subjects when class changes ────────────────────────────
    useEffect(() => {
    ...
    // ── Step 4: Fetch students for selected class ────────────────────────────
    const fetchStudents = useCallback(async () => {
        ...
        try {
            const res = await fetch(
                `${API_PATH}/api/students/?current_class=${selectedClasses.id}`,
                { credentials: "include" }
            );
    ...
    const handleSubmit = async () => {
        ...
        const submitData = {
            ...
            date: selectedDate,
            marked_by: currentUser?.teacher_profile_data?.teacher_id ?? 1,
    ...
    // ── Logout (mirrors Sidebar logic exactly) ───────────────────────────────
    ...
    const displayRole = isTeacher
        ? currentUser?.teacher_profile_data?.subject_specialization
            ? `${currentUser.teacher_profile_data.subject_specialization} Teacher`
            : "Teacher"
        : isAdmin
        ? "Administrator"
        : currentUser?.role ?? "";
    const schoolName =
        currentUser?.teacher_profile_data?.school?.name ?? "EduTrack School";
                ...
                <section className="nav min-h-[10vh] bg-gray-50 shadow-xl justify-between items-center flex flex-row p-3 px-4 sm:px-6 lg:px-10 sticky top-0 z-50">
                    ...
                    <div className="flex flex-row gap-3 lg:gap-6 justify-center items-center">
                        <div className="flex flex-col justify-center items-end sm:flex">
                            {/* Teacher/Admin name: dynamic from /me */}
                            <h3 className="text-gray-900 font-bold text-sm lg:text-lg leading-tight">
                                {displayName}
                            </h3>
                            <span className="text-gray-500 text-xs lg:text-sm">{displayRole}</span>
                        </div>
                    ...
                    <div className="register w-[95%] lg:w-[90%] bg-gray-50 rounded-xl p-4 lg:p-7 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:flex lg:flex-row lg:justify-between lg:items-end gap-4 lg:gap-6">
                            ...
                            <div className="flex flex-col gap-2">
                                <h4 className="text-xs font-bold text-gray-500 uppercase ml-3 tracking-wider flex items-center gap-1">
                                    Select Class
                                    {/* Lock icon shown to teachers to signal the field is fixed */}
                                    {isTeacher && <Lock size={11} className="text-gray-400" />}
                                </h4>

                                {isTeacher ? (
                                    /*
                                     * TEACHER: read-only pill — shows their class, cannot change.
                                     * If a teacher has multiple classes we show the first one locked
                                     * (edge case — most teachers have one homeroom class).
                                     */
                                    <div className="bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-gray-700 text-sm w-full select-none cursor-not-allowed flex items-center justify-between">
                                        <span>{selectedClasses?.name ?? "—"}</span>
                                        <Lock size={13} className="text-gray-400 shrink-0" />
                                    </div>
                                ) : (
                                    /*
                                     * ADMIN: full dropdown, all classes
                                     */
                                    <select
